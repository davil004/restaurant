...
public class LoginController {
	...
	@Nullable
	@RequestMapping(path = { "/pages/login.controller" })
...
//		System.out.println("-----------");
		...
		if (login != null && login.equals("登入")) {
			...
			String level; 
			if(memberService.getLevel(bean) > 10000) {
				level = "金牌會員";
			}else if(memberService.getLevel(bean) > 5000){
				level = "銀牌會員";
			}else if(memberService.getLevel(bean) > 2000){
				level = "銅牌會員";
			}else{
				level = "無牌會員";
			};
